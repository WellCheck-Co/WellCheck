...
class BplistService {
    ...
    parse64Content(data) {
        ...
        const array = Buffer.alloc(rawLength);
...
class TextService {
    constructor() {
        this.binaryHelperService = new BplistService();
    }
    identify(current) {
        return current._class === 'text';
    }
    lookup(current) {
...
class ImageService {
    ...
    identify(current) {
        return current._class === 'bitmap';
    }
    lookup(current, data) {
        return this.getImageDataFromRef(data, current.image._ref);
    }
    render(current, data, options) {
        const content = this.getImageDataFromRef(data, current.image._ref);
        ...
        return [
            {
                ...
                value: buf,
                language: 'binary',
                uri: `${options.assetDir}/${this.format.normalizeName(current.name)}.png`
            }
        ];
    }
    ...
    bitmapRegistryOf(data) {
        const legacyResourceRegistry = data.resources &&
            data.resources.images &&
            Object.entries(data.resources.images).reduce((acc, [relativePath, image]) => (Object.assign(Object.assign({}, acc), { [relativePath]: image.source })), {});
        return legacyResourceRegistry || data.images;
    }
}

class SymbolService {
    identify(current) {
        return current._class === 'symbolInstance';
    }
    lookup(current, data) {
        const foreignSymbol = data.document.foreignSymbols.find(x => x.symbolMaster.symbolID === current.symbolID);
        return foreignSymbol && foreignSymbol.symbolMaster;
    }
}

class ShapeService {
    parsePoint(point, offset, node) {
        ...
        return {
            ...
            y: Number.parseFloat((node.frame.height * Number.parseFloat(parsedPoint[1]) +
                offset).toFixed(3))
        };
    }
}
...
class CssContextService {
    ...
    put(current, nextContext = {
        ...
        current.css = Object.assign(Object.assign({}, this.of(current)), nextContext);
    }
    clear(current) {
        delete current.web;
    }
...
class CssParserService {
    constructor() {
        this.styleHelperService = new StyleService();
        this.cssContext = new CssContextService();
    ...
    visitLayerStyle(current) {
        this.cssContext.put(current, {
            rules: Object.assign(Object.assign(Object.assign(Object.assign({}, this.extractFrame(current)), this.extractRotation(current)), this.extractBorderRadius(current)), this.extractOpacity(current))
        });
    }
    visitRectangleStyle(current) {
        this.cssContext.put(current, {
            rules: Object.assign(Object.assign(Object.assign(Object.assign({}, this.extractFrame(current)), this.extractBorders(current)), this.extractFills(current)), this.extractShadows(current)),
            pseudoElements: {
    ...
    visitOvalStyle(current) {
        this.cssContext.put(current, {
            rules: Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, this.addOvalShape()), this.extractFrame(current)), this.extractBorders(current)), this.extractFills(current)), this.extractShadows(current)),
            pseudoElements: {
                before: this.extractBlurPseudoElement(current)
            }
        });
    ...
    visitTextStyle(current) {
        this.cssContext.put(current, {
            rules: Object.assign(Object.assign(Object.assign(Object.assign({}, this.extractFrame(current)), this.extractTextFont(current)), this.extractTextColor(current)), this.extractParagraphStyle(current))
        });
    }
    ...
    extractBlurPseudoElement(current) {
        ...
        if (obj && obj.hasOwnProperty('radius') && obj.radius > 0) {
            const objFill = current.style.fills;
    ...
    extractBorders(current) {
        let BorderType;
        (function (BorderType) {
            BorderType[BorderType["INSIDE"] = 1] = "INSIDE";
            BorderType[BorderType["OUTSIDE"] = 2] = "OUTSIDE";
            BorderType[BorderType["CENTER"] = 0] = "CENTER";
        ...
        if (obj && obj.length > 0) {
            ...
            if (firstFill.isEnabled) {
                ...
                if (blurObj && blurObj.hasOwnProperty('radius') && blurObj.radius > 0) {
                    return Object.assign(Object.assign({}, this.extractFillGradient(firstFill)), { background: 'inherit', overflow: 'hidden', 'background-color': fillColor });
                }
                else {
                    return Object.assign(Object.assign({}, this.extractFillGradient(firstFill)), { 'background-color': fillColor });
                }
            }
        }
        return {};
...
class CssRenderService {
            ...
            if (_current.layers && Array.isArray(_current.layers)) {
                _current.layers.forEach(layer => {
                    if (this.cssContext.identify(layer)) {
                        const name = `${layer.css.className}`;
                        if (layer.css.pseudoElements) {
                            Object.entries(layer.css.pseudoElements).forEach(([prop, value]) => {
                                content(`${name}:${prop}`, Object.entries(value).map(([ruleKey, ruleValue]) => `${ruleKey}: ${ruleValue};`));
                            });
                            computeStyle(layer);
                        }
                    }
    ...
    buildAstStyleSheet(styles, current) {
        const computeStyle = (_current, _styles) => {
            const content = (name, data) => {
                if (data) {
                    styles.push({ className: name, declarations: data });
                }
    ...
    postProcessCss(stylesAst) {
        ...
        for (let currentIndex = 0; currentIndex < stylesAst.length; currentIndex++) {
            ...
            while (++checkingDecIndex < stylesAst.length) {
                ...
                const checkDeclarationPropertySet = new Set(nextDeclaration.declarations.sort());
    ...
    reduceDuplicates(duplicates, stylesAst) {
        const deDuplicateCssValues = duplicates.reduce((current, next, index, _array) => {
     ...
     */
    setValuesInAst(stylesAst, currentIndex, currentDeclarationSet, checkingDecIndex, checkDeclarationPropertySet) {
...
class CssBlocGenService {
    constructor() {
        this.cssContext = new CssContextService();
        this.cssParser = new CssParserService();
        this.cssRender = new CssRenderService();
    }
    compute(current, data, options) {
...
class SvgContextService {
    identify(current) {
        return ['triangle', 'shapePath'].includes(current._class);
    }
    has(current) {
        return !!current.svg;
    }
    of(current) {
        return current.svg || { paths: [], offset: 0, attributes: [] };
    }
    put(current, newContext = { paths: [], offset: 0, attributes: [] }) {
        current.svg = Object.assign(Object.assign({}, this.of(current)), newContext);
    }
    clear(current) {
        delete current.svg;
    }
...
class SvgParserService {
    constructor() {
        this.shape = new ShapeService();
        this.layer = new LayerService();
        this.style = new StyleService();
        this.symbol = new SymbolService();
        this.svgContext = new SvgContextService();
    }
    compute(current, data, options) {
        this.visit(current, data, options);
    }
    ...
    visitShapePath(current) {
        ...
        const origin = this.shape.parsePoint(current.points[0].point, offset, current);
        const segments = current.points.slice(1).map(curvePoint => {
    ...
    visitShapeGroup(current) {
        ...
        const paths = current.layers.map(layer => {
            // TODO: move to @types/sketchapp
            const origin = this.shape.parsePoint(layer.points[0].point, offset, layer);
            const segments = layer.points.slice(1).map(curvePoint => {
...
${openTag}>
${paths
            .map(path => this.format.indent(1, `<${path.type} ${path.attributes.join(' ')}/>`))
            .join('\n')}
</svg>`;
    }
...
class SvgBlocGenService {
    constructor() {
        this.svgContext = new SvgContextService();
        this.svgParser = new SvgParserService();
        this.svgRender = new SvgRenderService();
    }
    compute(current, data, options) {
...
export { Buffer as B, CssBlocGenService as C, FormatService as F, ImageService as I, LayerService as L, SymbolService as S, TextService as T, SvgBlocGenService as a, global$1 as g };
